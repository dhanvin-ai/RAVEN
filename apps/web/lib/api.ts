...
// ============================================================
// TELEMETRY OBSERVE (Phases 4+5 — /v1 production telemetry)
// Key is per-project; stored in localStorage by the Observe UI.
// ============================================================

export function telemetryHeaders(apiKey: string): Record<string, string> {
  return { "X-RAVEN-API-KEY": apiKey };
}

export interface TelemetryConversationItem {
  id: number;
  external_id: string;
  status: string;
...
export interface TelemetryOverview {
  ...
  cost_usd: number;
  timeseries: { day: string; conversations: number; traces: number; errors: number; p95_latency_ms: number | null }[];
  tools: { tool: string; calls: number; errors: number; error_rate: number }[];
  models: { model: string | null; provider: string | null; traces: number; errors: number; tokens: number; cost_usd: number; avg_latency_ms: number | null }[];
  versions: { version: string; traces: number; errors: number; error_rate: number }[];
  top_errors: { error_type: string; count: number; sample: string | null }[];
}

export interface TelemetryActivityItem {
  kind: string;
  id: number;
  trace_id?: string;
  event_id?: string;
...
function telemetryQuery(params: Record<string, string | number | boolean | undefined>): string {
...
export async function getTelemetryOverview(apiKey: string, days = 7): Promise<TelemetryOverview> {
  return request<TelemetryOverview>(`/v1/analytics/overview${telemetryQuery({ days })}`, {
    headers: telemetryHeaders(apiKey),
  });
}

export async function listTelemetryConversations(
  apiKey: string,
  params: Record<string, string | number | boolean | undefined> = {}
): Promise<{ success: boolean; total: number; items: TelemetryConversationItem[] }> {
  return request(`/v1/conversations${telemetryQuery(params)}`, {
    headers: telemetryHeaders(apiKey),
  });
}

export async function getTelemetryConversation(apiKey: string, id: number): Promise<TelemetryConversation> {
  return request<TelemetryConversation>(`/v1/conversations/${id}`, {
    headers: telemetryHeaders(apiKey),
  });
}
...
export async function listTelemetryEvents(
  apiKey: string,
  params: Record<string, string | number | boolean | undefined> = {}
): Promise<{ success: boolean; total: number; items: (TelemetryEventItem & { payload: Record<string, unknown>; trace_id: number | null; conversation_id: number | null })[] }> {
  return request(`/v1/events${telemetryQuery(params)}`, {
    headers: telemetryHeaders(apiKey),
  });
}

export async function listTelemetryToolCalls(
  apiKey: string,
  params: Record<string, string | number | boolean | undefined> = {}
): Promise<{
  success: boolean;
  total: number;
  summary: { tool: string; calls: number; errors: number; error_rate: number }[];
  items: { id: number; tool: string; outcome: string; latency_ms: number | null; trace_id: string; conversation_id: number | null; arguments: Record<string, unknown>; result: Record<string, unknown>; started_at: string | null }[];
}> {
  return request(`/v1/tool-calls${telemetryQuery(params)}`, {
    headers: telemetryHeaders(apiKey),
  });
}
...
export async function getTelemetryActivity(
  apiKey: string,
  sinceId = 0
): Promise<{ success: boolean; items: TelemetryActivityItem[]; max_id: number }> {
  return request(`/v1/activity${telemetryQuery({ since_id: sinceId })}`, {
    headers: telemetryHeaders(apiKey),
  });
}
